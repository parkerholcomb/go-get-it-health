...
class Fetcher:
    def __init__(self, source, fetch = True):
        self.source = source
    ...
    def fetch_latest(self):
        ...
        df.to_csv(f's3://data-{self.source}/raw/{today}/{hr_min}/{self.source}-vaccine-supply.csv', index=False)
        df.to_csv(f's3://data-{self.source}/raw/latest/{self.source}-vaccine-supply.csv', index=False)
        print(f"fetched latest from {self.source}")
